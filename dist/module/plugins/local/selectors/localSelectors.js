...
var textFilterRowSearch = function textFilterRowSearch(row, columnProperties, filter) {
...
var objectFilterRowSearch = function objectFilterRowSearch(row, columnProperties, filter) {
  return row.keySeq().some(function (key) {
    var filterable = columnProperties && columnProperties.getIn([key, 'filterable']);
    if (filterable === false) {
      return false;
